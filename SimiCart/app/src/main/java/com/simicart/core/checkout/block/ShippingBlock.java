...
public class ShippingBlock extends SimiBlock implements ShippingDelegate {
        ...
        if (null != list_shippingMethods) {
            ...
            for (int i = 0; i < list_shippingMethods.size(); i++) {
                ...
                if (incl_tax == null || incl_tax.equals("")
                    ...
                    tx_price.setText(AppStoreConfig.getInstance().getPrice(price));
                    tx_price.setTextColor(AppColorConfig.getInstance().getPriceColor());
                } else {
                    String price_method = "<font  color='" + AppColorConfig.getInstance().getPriceColor() + "'>"
                            + AppStoreConfig.getInstance().getPrice(price)
                            + "</font> <font color='" + AppColorConfig.getInstance().getContentColor() + "'>+"
                            + "(" + SimiTranslator.getInstance().translate("Incl. Tax")
                            + "</font> <font  color='" + AppColorConfig.getInstance().getPriceColor() + "'> "
                            + AppStoreConfig.getInstance().getPrice(incl_tax)
                            + "</font>" + "<font color='" + AppColorConfig.getInstance().getContentColor() + "'>" + ")" + "</font>";
                ...
                Drawable icon_nomal = mContext.getResources().getDrawable(
                        mIDIconNormal);
                icon_nomal.setColorFilter(AppColorConfig.getInstance().getContentColor(),
                ...
                if (list_shippingMethods.get(i).isS_method_selected()) {
                    ...
                    icon_checked.setColorFilter(AppColorConfig.getInstance().getContentColor(), PorterDuff.Mode.SRC_ATOP);
    ...
    protected void requestSelectShippingMethod(
        ...
        if (!shippingMethod.isS_method_selected()) {
            ...
            mModel.addBody("s_method_id", shippingMethod.getS_method_id());
            mModel.addBody("s_method_code", shippingMethod.getS_method_code());
            mModel.request();
        }
    }
