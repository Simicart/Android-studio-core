...
public class DownloadingAdapter extends BaseAdapter {
	...
	public View getView(final int position, View convertView, ViewGroup arg2) {
		...
		if (cur.moveToFirst()) {
			holder.tv_downloading_file_name.setText(SimiTranslator.getInstance().translate("File Title: ")
			...
			new Thread(new Runnable() {
				...
				public void run() {
					...
					while (downloading) {
						...
						SimiManager.getIntance().getCurrentActivity().runOnUiThread(new Runnable() {

							@Override
							public void run() {
								holder.tv_downloading_file_size.setText(SimiTranslator.getInstance().translate("File Size: "
										+ new DecimalFormat("##.##").format(bytes_total / (Math.pow(1024, 2))) + "MB"));
								holder.tv_downloading_file_downloaded.setText(SimiTranslator.getInstance()
