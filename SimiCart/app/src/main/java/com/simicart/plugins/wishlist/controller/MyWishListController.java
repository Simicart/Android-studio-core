...
public class MyWishListController extends SimiController {
    ...
    public void controllerShare(String sharing_mes) {
        if (sharing_mes != null && !sharing_mes.equals("")) {
            ...
            SimiManager.getIntance().getCurrentActivity().startActivity(Intent.createChooser(intent2,
                    "Share via"));
        }
    }
    ...
    private void getMyWishList() {
        ...
        mModel.setFailListener(new ModelFailCallBack() {
            ...
            public void onFail(SimiError error) {
                mDelegate.dismissLoading();
                SimiNotify.getInstance().showToast("Error! Please try again.");
            }
        });
        mModel.addBody("limit", String.valueOf(limit));
        mModel.addBody("offset", String.valueOf(mCurrentOffset));
        mModel.request();
    }
